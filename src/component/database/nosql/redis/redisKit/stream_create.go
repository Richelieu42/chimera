...
// XGroupCreate 创建消费者组Consumer Group.
/*
命令: ("xgroup", "create", stream, group, start)

@return (1) 如果 stream 不存在的话，会返回error:
				ERR The XGROUP subcommand requires the key to exist. Note that for CREATE you may want to use the MKSTREAM option to create an empty stream automatically.
		(2) stream 存在的情况下，如果 group 已经存在的话，会返回error（此种error建议忽略; 可以通过 IsConsumerGroupNameAlreadyExistError 判断）:
...
// XGroupCreateMkStream 创建消费者组Consumer Group（如果stream不存在，那么会自动创建一个空（长度为0）的stream）.
/*
PS:
(1) 命令: ("xgroup", "create", stream, group, start, "mkstream")
	MKSTREAM是一个可选子命令，如果指定了它，那么在创建消费者组的时候，如果stream不存在，那么会自动创建一个空（长度为0）的stream

@return stream 存在的情况下，如果 group 已经存在的话，会返回error（此种error建议忽略; 可以通过 IsConsumerGroupNameAlreadyExistError 判断）:
				BUSYGROUP Consumer Group name already exists
